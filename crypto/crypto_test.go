...
func TestScalarMultCommute(t *testing.T) {
	a := NewRandomScalar()
	b := NewRandomScalar()

	P := a.MultG().ScalarMult(b)
	Q := b.MultG().ScalarMult(a)

	if P.Equal(Q) != 1 {
		t.Errorf("Failed ! abG != baG")
	}
}

func TestAddCommute(t *testing.T) {
	A := NewRandomScalar().MultG()
	B := NewRandomScalar().MultG()

	P := A.Add(B)
	Q := B.Add(A)

	if P.Equal(Q) != 1 {
		t.Errorf("Failed ! A + B != B + A")
	}
}
...
func TestBaseMult(t *testing.T) {
	a := NewRandomScalar()
	P := a.MultG()
	Q := PointG().ScalarMult(a)

	if P.Equal(Q) != 1 {
		t.Errorf("Failed ! PointG().ScalarMult(a) != ScalarBaseMult(a)")
	}
}

func TestNotEqual(t *testing.T) {

	P := NewRandomScalar().MultG()
	Q := NewRandomScalar().MultG()
...
func TestScalarMultiplySubtract(t *testing.T) {
	tests := []struct {
		name    string
		aHex    string
		bHex    string
	...
	for _, test := range tests {
		...
		a := NewScalarFromHexString(test.aHex)
		b := NewScalarFromHexString(test.bHex)
		c := NewScalarFromHexString(test.cHex)
		want := NewScalarFromHexString(test.wantHex)

		got := a.MultiplyAdd(b.Negate(), c)

		if want.Equal(got) == 0 {
			t.Errorf("%s: want %x, got %x", test.name, want.Bytes(), got.Bytes())
		}
	}
}
...
func TestScalarMultiply(t *testing.T) {
	...
	for _, test := range tests {
		a := NewScalarFromHexString(test.scalarHex)
		P := NewPointFromHexString(test.pointHex)
		want := NewPointFromHexString(test.wantHex)
...
func TestDoubleScalarBaseMult(t *testing.T) {
	...
	for _, test := range tests {
		P := NewPointFromHexString(test.pointHex)
		a := NewScalarFromHexString(test.scalar1Hex)
		b := NewScalarFromHexString(test.scalar2Hex)
		want := NewPointFromHexString(test.wantHex)

		got := b.DoubleScalarBaseMult(a, P)
...
func TestScalarEncodingValidity(t *testing.T) {
	...
	for _, test := range tests {
		...
		if test.valid {
			if a.Err != nil {
				t.Errorf("%x: want %t, got %t", test.scalarHex, test.valid, a.Err)
			}
		} else {
			if a.Err == nil {
				t.Errorf("%x: want %t, got %t", test.scalarHex, test.valid, a.Err)
...
func TestScalarToBytesToScalar(t *testing.T) {
	want := NewRandomScalar()
	aBytes := want.Bytes()
	got := NewScalarFromBytes(aBytes)
	if want.Equal(got) == 0 {
		t.Errorf("ScalarToBytesToScalar failed: want %x, got %x", want.Bytes(), got.Bytes())
	}
}

func TestPointToBytesToPoint(t *testing.T) {
	want := NewRandomScalar().MultG()
	ABytes := want.Bytes()
	got := NewPointFromBytes(ABytes)
	if want.Equal(got) == 0 {
		t.Errorf("PointToBytesToPoint failed: want %x, got %x", want.Bytes(), got.Bytes())
	}
}

//validates Scalar Multiply assuming other functions are valid
func TestMultiply(t *testing.T) {
	for i := 0; i < 5; i++ {
		...
		//check if b * (a * G) == (a * b) * G
		if baG.Equal(abG) == 0 {
			t.Errorf("Multiply failed: want %x, got %x", baG.Bytes(), abG.Bytes())
		}
	}
}

// ScalarEight returns a new Scalar set to 8.
func ScalarEight() (r *Scalar) {
	r = new(Scalar)
	r.edScalar, _ = new(edwards25519.Scalar).SetCanonicalBytes([]byte{0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00})
	return
}

func TestPoint_MultByCofactor(t *testing.T) {
	r := NewRandomScalar()
	want := r.Multiply(ScalarEight()).MultG()
	got := r.MultG().MultByCofactor()
	if want.Equal(got) == 0 {
		t.Errorf("MultByCofactor failed: want %x, got %x", want.Bytes(), got.Bytes())
	}
}

func BenchmarkScalar_MultG(b *testing.B) {
	r := NewRandomScalar()
	b.ResetTimer()
	for n := 0; n < b.N; n++ {
		_ = r.MultG()
...
func BenchmarkScalar_MultPoint_G(b *testing.B) {
	r := NewRandomScalar()
	b.ResetTimer()
	for n := 0; n < b.N; n++ {
		_ = r.MultPoint(PointG())
	}
}

func Benchmark_MultByCofactor(b *testing.B) {
	r := NewRandomScalar().MultG()
	b.ResetTimer()
	for n := 0; n < b.N; n++ {
		_ = r.MultByCofactor()
	}
...
func Benchmark_ScalarMult8(b *testing.B) {
	r := NewRandomScalar().MultG()
	eight := ScalarEight()
	b.ResetTimer()
	for n := 0; n < b.N; n++ {
		_ = r.ScalarMult(eight)
	}
}
...
func BenchmarkPoint_Add(b *testing.B) {
	a1 := NewRandomScalar().MultG()
	a2 := NewRandomScalar().MultG()
	b.ResetTimer()
	for n := 0; n < b.N; n++ {
		_ = a1.Add(a2)
	}
